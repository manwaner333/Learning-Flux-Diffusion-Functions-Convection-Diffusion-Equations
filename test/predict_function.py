...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def a_real_1(self, u):
        a = (0.13043321654953507)*u+(-0.027944399874594957)*1
        return torch.abs(a).double()

    def a_real_2(self, u):
        a = (-0.24087143090560018)*1+(-0.18124830505654063)*u
        return torch.abs(a).double()

    def a_real_3(self, u):
        a = (1.1067946866719238)*u+(0.22872482452362475)*1
        return torch.abs(a).double()

    def a_real_4(self, u):
        a = (-0.6932061764943792)*u+(-0.35417742489838144)*1
        return torch.abs(a).double()
...
if __name__ == "__main__":
    experiment_name = 'beta_300_case_1_0.05'
