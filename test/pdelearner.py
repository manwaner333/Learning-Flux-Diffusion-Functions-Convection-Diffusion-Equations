...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def f_real(self, u):
        ...
        f = 0.05 * torch.pow(u, 2) / (torch.pow(u, 2) + 0.5 * torch.pow(1-u, 2))
        return f
    ...
    def a_max(self, u):
        if self.is_train:
            a = 0.01 * self.a_predict(u)
        else:
            a = 0.01 * self.a_real(u)
        return torch.max(a)

    def A_f_half(self, u):
        if self.is_train:
            f = self.f_predict(u)
            a = 0.01 * self.a_predict(self.u_a)
        else:
            f = self.f_real(u)
            a = 0.01 * self.a_real(self.u_a)
        f_left = f[:, 0:self.N - 1]
        f_right = f[:, 1:self.N]
        u_left = u[:, 0:self.N - 1]
        u_right = u[:, 1:self.N]
    ...
    def update(self):
        ...
        if 0.0 < max_f_prime < 25.0 and 0.0 < amax < 1.0:
