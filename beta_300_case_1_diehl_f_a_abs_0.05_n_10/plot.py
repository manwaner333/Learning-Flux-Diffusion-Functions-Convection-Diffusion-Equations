...
def a_predict_function(u):
    c = [-0.0865, -0.1681, -0.0662, -0.3089, -0.1723, -0.2125, -0.3563,  0.0566,
          0.3912, -0.0923, -0.1030]
    if -0.1 < u <= 0:
        res = c[0] * psi_0_l(u)
    elif 0 < u <= 0.1:
...
def f_predict_function(u):
    c = [-0.9357, -0.6516, -0.2366,  0.1399,  0.3710,  0.4455,  0.3986,  0.2627,
          0.1232,  0.0593,  0.0532]
    if -0.1 < u <= 0:
        res = c[0] * psi_0_l(u)
    elif 0 < u <= 0.1:
        res = c[0] * psi_0_r(u) + c[1] * psi_1_l(u)
...
if __name__ == "__main__":
    N = 400
    test_data_name = 'beta_300_case_1_0.05_diehl'
