...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def a_real_1(self, u):
        a = (0.07815283776199615)*u+(-0.012833615661223213)*1
        return torch.abs(a).double()

    def a_real_2(self, u):
        a = (-0.09564055084665812)*u+(0.044796810513157176)*1
        return torch.abs(a).double()

    def a_real_3(self, u):
        a = (1.176037326938302)*u+(0.36326542720795696)*1
        return torch.abs(a).double()

    def a_real_4(self, u):
        a = (-0.6193891049650374)*u+(-0.26901743489649793)*1
        return torch.abs(a).double()
...
if __name__ == "__main__":
    experiment_name = 'beta_300_case_1_N_200_0.5_obs_2'
