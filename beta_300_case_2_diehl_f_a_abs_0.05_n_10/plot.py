...
def a_predict_function(u):
    c = [-0.0837, -0.1649, -0.0895, -0.3282, -0.1738, -0.2018, -0.3456,  0.0521,
          0.3771, -0.0916, -0.1002]
    if -0.1 < u <= 0:
        res = c[0] * psi_0_l(u)
    elif 0 < u <= 0.1:
...
def f_predict_function(u):
    c = [-0.9295, -0.6586, -0.2325,  0.1398,  0.3702,  0.4351,  0.3750,  0.2558,
          0.1305,  0.0742,  0.0695]
    if -0.1 < u <= 0:
        res = c[0] * psi_0_l(u)
    elif 0 < u <= 0.1:
        res = c[0] * psi_0_r(u) + c[1] * psi_1_l(u)
