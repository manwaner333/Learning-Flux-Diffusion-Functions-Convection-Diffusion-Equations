...
def f_predict_function(u):
    ...
    f = (1.6007584810799407)*u+(-0.9883314701787949)*1+(-0.5530206794614075)*u**3+(-0.10650004748732356)*u**2+(0.04265457370453625)*u**5+(0.01375488948658176)*u**4+(-0.0019191470512096528)*u**6+(-0.0003796431316146885)*u**7+(-9.513165823965499e-06)*u**8
    return f
...
def a_predict_function(u):
    if u <= 0.25:
        y = (-0.022682899907106292)*1+(-0.0011738279368777983)*u
    elif u > 0.25 and u < 0.5:
        y = (-0.09031556512311499)*u+(-0.018738520282332298)*1
    elif u >= 0.5 and u < 0.75:
        y = (-0.18559242860941805)*u+(0.025331690648030092)*1
    else:
        y = (0.23889011554362097)*u+(0.08688314908105284)*1
    return np.abs(y)
...
if __name__ == "__main__":
    ...
    test_data_name = 'beta_10_case_2_0.05'
