...
def f_predict_function(u):
    a = (-14.365864876914483)*u**3+(12.392969065843888)*u**6+(10.651975767034985)*u**4+(-9.921092779014636)*u**7+(-7.368589800919859)*u**5+(6.332999543542599)*u+(3.0102323072589456)*u**8+(-2.160678088276108)*1+(-0.3914339001168976)*u**2
    b = (-19.072817526176234)*u**6+(15.268592314647712)*u**7+(11.340282377168817)*u**5+(6.34631639182162)*u**2+(-6.056065108801775)*u**3+(-4.63275678352069)*u**8+(-2.276584514192802)*u+(1.3184765617100216)*1+(0.6981858180360465)*u**4
    return a/b
...
def a_predict_function(u):
    if u <= 0.25:
        y = (0.09131162290939467)*u+(-0.016460472202208334)*1
    elif u > 0.25 and u < 0.5:
        y = (0.5305696535490058)*u+(-0.2013079925877098)*1
    elif u >= 0.5 and u < 0.75:
        y = (-0.8448599276947449)*1+(-0.1584631248152882)*u
    else:
        y = (1.1133503810333285)*1+(0.21372050667675374)*u
    return np.abs(y)
