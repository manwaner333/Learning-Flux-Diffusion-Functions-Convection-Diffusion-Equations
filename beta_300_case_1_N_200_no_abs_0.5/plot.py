...
def plot_fixed_time_step_different_beta_0_1(real_0_data, real_1_data, real_0_step, real_1_step, i, time, N, example_index, name):
    ...
    time_steps_1 = [0, 65, 129, 194, 259, 323, 388, 452, 517, 582]
    # for i in range(1, 2):  #len(time_steps_0)
...
def plot_real_and_predict_a_function_different_beta(name='name'):
    x = []
    N = 100
    dx = 1.0/N
    for i in range(N + 1):
        x.append(i*dx)
    f_0 = np.array([0 * a_real_function(ele) for ele in x])
    f_1 = np.array([0.1 * a_real_function(ele) for ele in x])
    f_3 = np.array([0.3 * a_real_function(ele) for ele in x])
    f_5 = np.array([0.5 * a_real_function(ele) for ele in x])
    f_8 = np.array([0.8 * a_real_function(ele) for ele in x])
    f_10 = np.array([1 * a_real_function(ele) for ele in x])
    ...
    fig = plt.figure(figsize=(40, 20))
    # plt.plot()
    plot_0, = plt.plot(x, f_0, label='observation', color='red', linestyle='-', linewidth=linewith)
    plot_1, = plt.plot(x, f_1, label='observation', color='blue', linestyle='-', linewidth=linewith)
    plot_3, = plt.plot(x, f_3, label='observation', color='green', linestyle='-', linewidth=linewith)
    plot_5, = plt.plot(x, f_5, label='observation', color='yellow', linestyle='-', linewidth=linewith)
    plot_8, = plt.plot(x, f_8, label='observation', color='cyan', linestyle='-', linewidth=linewith)
    plot_10, = plt.plot(x, f_10, label='observation', color='magenta', linestyle='-', linewidth=linewith)
...
def plot_real_and_predict_A_function_different_beta(name='name'):
    ...
    for i in range(N + 1):
        x.append(i * dx)
    A_real = A_real_function()
    f_0 = np.array([0 * ele for ele in A_real])
    f_1 = np.array([0.1 * ele for ele in A_real])
    f_3 = np.array([0.3 * ele for ele in A_real])
    f_5 = np.array([0.5 * ele for ele in A_real])
    f_8 = np.array([0.8 * ele for ele in A_real])
    f_10 = np.array([1 * ele for ele in A_real])
    ...
    ax1.spines['right'].set_linewidth(linewith_frame)
    ax1.spines['left'].set_linewidth(linewith_frame)
    plt.legend(handles=[plot_0, plot_1, plot_3, plot_5, plot_8, plot_10],  # ,  plot_2
               labels=['0', '0.1', '0.3', '0.5', '0.8', '1.0'],  # 'prediction',
               loc="upper left", fontsize=50, frameon=True, edgecolor='green')
    plt.grid(True, which='major', linewidth=0.1, linestyle='--')
    # ax1.set_facecolor('w')
    plt.show()
...
if __name__ == "__main__":
    ...
    data_1_file = 'data/beta_300_case_1_N_200_0.1_U.npy'
    data_3_file = 'data/beta_300_case_1_N_200_0.3_U.npy'
    data_5_file = 'data/beta_300_case_1_N_200_0.5_U.npy'
    data_8_file = 'data/beta_300_case_1_N_200_0.8_U.npy'
    data_10_file = 'data/beta_300_case_1_N_200_1.0_U.npy'

    real_0_step = 226  # 113  # 226
    real_1_step = 440  # 220  # 440
    real_3_step = 867  # 433  # 867
    real_5_step = 1293  # 646  # 1293
    real_8_step = 1933  # 966  # 1933
    real_10_step = 2360  # 1180  # 2360

    time = 2
    real_0_data = np.load(data_0_file)
    real_1_data = np.load(data_1_file)
    real_3_data = np.load(data_3_file)
    real_5_data = np.load(data_5_file)
    real_8_data = np.load(data_8_file)
    real_10_data = np.load(data_10_file)


    plot_fixed_time_step_different_beta(real_0_data, real_1_data, real_3_data, real_5_data, real_8_data, real_10_data,
                                        real_0_step, real_1_step, real_3_step, real_5_step, real_8_step, real_10_step, time, N, example_index, name=test_data_name)

    # f(u)函数的差别with different diffusion
    # plot_real_and_predict_a_function_different_beta(name=test_data_name)
    # plot_real_and_predict_A_function_different_beta(name=test_data_name)
    example_index = 1
    time_steps_0 = [0, 11, 23, 34, 45, 57, 68, 79, 91, 102]
    time_steps_5 = [0, 65, 129, 194, 259, 323, 388, 452, 517, 582]
    time_step = 5
    real_0_step = time_steps_0[time_step]
    real_5_step = time_steps_5[time_step]

    # plot_fixed_time_step_different_beta_0_1(real_0_data, real_5_data, real_0_step, real_5_step, time_step, time, N,
    #                                         example_index, name=test_data_name)

    # 比较每一段u上的误差大小
    data_0_file = 'data/beta_300_case_1_N_200_0.5_without_diff_U.npy'
    data_5_file = 'data/beta_300_case_1_N_200_0.5_U.npy'
    real_0_data = np.load(data_0_file)[real_0_step, example_index, :]
    real_5_data = np.load(data_5_file)[real_5_step, example_index, :]
    where_res_0 = np.where(real_0_data >= 0.75)
    where_res_5 = np.where(real_5_data >= 0.75)
    res_0 = real_0_data[where_res_0]
    res_5 = real_5_data[where_res_5]

    x1 = where_res_0[0] * 0.05
    x2 = where_res_5[0] * 0.05
    y1 = res_0
    y2 = res_5

    # line1 = LineString([(x, y) for x, y in zip(x1, y1)])
    # line2 = LineString([(x, y) for x, y in zip(x2, y2)])
    #
    # coords = []
    ...
    #
    # area = polygon.convex_hull.area
    # print(area)
    #
    # # 提取凸包的坐标
    # hull_coords = polygon.convex_hull.exterior.coords.xy
    #
    # # 绘制凸包
    # fig, ax = plt.subplots()
    # ax.plot(hull_coords[0], hull_coords[1], '-o')
    # ax.set_aspect('equal')
    # plt.show()
    ...
    # ax.plot(x1, y1)
    # ax.plot(x2, y2)
    # ax.fill_between(x2, y1[0:33], y2, alpha=0.2)
    # plt.show()
    #
    # qingli = 3
