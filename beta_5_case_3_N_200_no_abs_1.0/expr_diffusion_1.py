...
class poly(torch.nn.Module):
    def __init__(self, hidden_layers, channel_num, channel_names=None, normalization_weight=None):
        ...
        for k in range(self.hidden_layers):
            module = torch.nn.Linear(self.channel_num+k, 2).to(dtype=torch.float64)
            module.weight.data.fill_(0)
            module.bias.data.fill_(0)
            self.add_module('layer_1'+str(k), module)
            layer.append(self.__getattr__('layer_1'+str(k)))
        module = torch.nn.Linear(self.channel_num + self.hidden_layers, 1).to(dtype=torch.float64)
        module.weight.data.fill_(0)
        module.bias.data.fill_(0)
        self.add_module('layer_final_1', module)
        layer.append(self.__getattr__('layer_final_1'))
        self.layer = tuple(layer)
        nw = torch.ones(self.channel_num).to(dtype=torch.float64)
        if (not isinstance(normalization_weight, torch.Tensor)) and (not normalization_weight is None):
            ...
            normalization_weight = normalization_weight.view(self.channel_num)
