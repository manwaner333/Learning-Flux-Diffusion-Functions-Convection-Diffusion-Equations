...
def loss(model, stepnum, obs_data, layerweight=None):
    ...
    x0_file = 'data/beta_5_case_3_N_200_0.0_x0.npy'
    ...
    U_ODE_choose = torch.empty((len(obs_t), model.batch_size, model.N), requires_grad=False, device=model.device)
    xi_old = x0_data
    for i in range(len(obs_t)):
        if obs_t[i] == 0:
            obs_data_choose[i, :, :] = xi_old
            U_ODE_choose[i, :, :] = obs_data[0, :, :]
        else:
            ads_U = specific_time_solution(obs_data, xi_old, obs_t[i], dt_fixed, x0_data)
            xi = 1.0 * ads_U * 0.1 + xi_old
            # xi = 1 * ads_U * 0.05 + xi_old
            xi_old = xi
            obs_data_choose[i, :, :] = xi_old
            U_ODE_choose[i, :, :] = ads_U
    # np.save('data/data_choose_trajectory', U_ODE_choose.detach().to('cpu'))
    ...
    for i in range(len(obs_t)):
        if obs_t[i] == 0:
            ...
            xi = 1.0 * ads_U_pre * 0.1 + xi_old
            # xi = 1 * ads_U_pre * 0.05 + xi_old
            xi_old = xi
            pre_data_choose[i, :, :] = xi_old
    ...
    # 位置观测点
    obs_points = []
    for i in range(0, 399, 10):  # 5
        obs_points.append(i)

    loss = 0
    max_f_prime_loss = 0
    fmax_up_bound = 100.0
    if 0 < max_f_prime and max_f_prime < fmax_up_bound:
        print("mormal")
        dataloss = mse(obs_data_choose[:, :, :], pre_data_choose[:, :, :])
        loss = dataloss
    elif max_f_prime >= fmax_up_bound and penalty > 0:
        print("penalty#loss")
        loss = penalty
    else:
        print("f#a#loss")
        print("\033[32mmax_f_prime %.6f, \033[0m" % (max_f_prime))
        max_f_prime_loss = max_f_prime - fmax_up_bound
