...
def plot_fixed_time_step(real_data, predict_data, time_steps_real, time_steps_predict, time, N, example_index, name):
    ...
    if not os.path.isdir(save_dir):
        os.mkdir(save_dir)
    file_name = '/' + 'fixed_t_s_r_p_d' + '_' + 'example' + '_' + str(example_index) + '_time_' + str(time) + '.pdf'
...
def f_predict_function(u):
    f = (1.5538220627892751)*u+(-1.0025569555175693)*1+(-0.6185582543975151)*u**3+(0.054578072389508105)*u**5+(0.016080185635206676)*u**2+(-0.010232617049814707)*u**4+(-0.0018941066874203745)*u**6+(-0.0005565949236244507)*u**7+(-1.6418481460394523e-05)*u**8
    return f
...
def a_predict_function(u):
    if u <= 0.25:
        y = (-0.41902797524712176)*1+(-0.07227990262656185)*u
    elif u > 0.25 and u < 0.5:
        y = (0.299421020098791)*1+(0.017911300321820436)*u
    elif u >= 0.5 and u < 0.75:
        y = (0.4606962832344054)*1+(0.06340275976798003)*u
    else:
        y = (0.7588824549514458)*u+(0.7177911227245899)*1
    return np.abs(y)
...
def plot_real_and_predict_A_function(name='name'):
    ...
    if not os.path.isdir(save_dir):
        os.mkdir(save_dir)
    file_name = '/' + 'r_p_A_u_function' + '.pdf'
    fig.savefig(save_dir + file_name, bbox_inches='tight', pad_inches=0.5)
