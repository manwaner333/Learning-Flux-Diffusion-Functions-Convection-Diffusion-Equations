...
options = {
    '--device': 'cpu',  # 'cuda:0',
    '--precision': 'double',
    '--taskdescriptor': 'N_400_example_6_mul_test_big_1',
    '--batch_size': 6,  # 250,  # 150,
    '--maxiter': 100,   # 156,
    '--X': 10,
    '--T': 2.0,
    '--dx': 0.025,   # 0.05,  # 0.025, # 0.05
    '--N': 400,   # 200  # cell number
...
def initexpr(model):
    ...
    for poly in rhi:
        for p in poly.parameters():
            p.data = torch.randn(*p.shape, dtype=p.dtype, device=p.device) * 1e-1 * 2
    spm = model.diffs
    for diff in spm:
