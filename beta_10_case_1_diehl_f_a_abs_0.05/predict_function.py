...
class VariantCoeLinear1d(torch.nn.Module):
    def __init__(self, T, N, X, batch_size, u0, dt, time_steps, dx, max_f_prime, u_fixed, u_a, N_a, theta, device,
        ...
        for k in range(self.channel_num):
            self.add_module('poly' + str(k), expr_m.poly(self.hidden_layers, channel_num=len(self.allchannels),
    ...
    def a_real_1(self, u):
        a = (-0.2541153094680921)*1+(-0.044654812412843185)*u
        a = torch.where(a >= 0.0, a, 0.0)
        return torch.abs(a).double()

    def a_real_2(self, u):
        a = (-1.705523166785545)*u+(0.21694638771144897)*1
        a = torch.where(a >= 0.0, a, 0.0)
        return torch.abs(a).double()

    def a_real_3(self, u):
        a = (0.6650958130539634)*1+(0.5863984157195072)*u
        a = torch.where(a >= 0.0, a, 0.0)
        return torch.abs(a).double()

    def a_real_4(self, u):
        a = (0.5389607495604511)*u+(0.4999398992088988)*1
        a = torch.where(a >= 0.0, a, 0.0)
        return torch.abs(a).double()
...
if __name__ == "__main__":
    experiment_name = 'N_400_example_4_mul_test_big_no'
