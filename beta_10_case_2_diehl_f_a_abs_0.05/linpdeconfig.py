...
def loss(model, stepnum, obs_data, layerweight=None):
    ...
    # amax
    a = 0.05 * model.a_predict(u_fixed_a)
    amin = torch.min(a)
    amax = torch.max(a)
    # aloss
    ...
    amax_up_bound = 0.5
    if 0 < max_f_prime < 5 and 0 < amax < amax_up_bound:
        print("mormal")
        loss = mse(obs_data_choose[:, :, :], pre_data_choose[:, :, :])
    elif 0 < max_f_prime < 5 and (amax <= 0.0 or amax >= amax_up_bound):
        print("a#loss")
        loss = torch.abs(amax - amax_up_bound) + torch.abs(amax)
    elif (max_f_prime <= 0 or max_f_prime >= 5) and 0.0 < amax < amax_up_bound:
        print("f#loss")
        loss = torch.abs(max_f_prime - 5) + torch.abs(max_f_prime)
    else:
        print("f#a#loss")
        loss = torch.abs(max_f_prime - 5) + torch.abs(max_f_prime) + torch.abs(amax - amax_up_bound) + torch.abs(amax)

    print("\033[33m loss0 %.6f, max_f_prime %.6f, amin %.6f, amax %.6f, a_loss %.6f \033[0m" % (loss, max_f_prime, amin, amax, aloss))
