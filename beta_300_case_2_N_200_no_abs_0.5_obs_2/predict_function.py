...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def f_real(self, u):
        # f = (-42797.389425858775)*u**22+(42581.72981556505)*u**23+(41307.17748492381)*u**21+(-40807.49010081839)*u**24+(-37869.73462537251)*u**20+(37602.739023978575)*u**25+(-33188.23126264582)*u**26+(32208.178111678513)*u**19+(27937.80802298891)*u**27+(-24250.08866634702)*u**18+(-22348.321843277037)*u**28+(16939.85305976747)*u**29+(14493.646323213954)*u**17+(-12141.801619831507)*u**30+(8611.2090992748)*u**14+(8216.530035778997)*u**31+(-8176.502183677926)*u**13+(-5242.854945716799)*u**32+(-4308.551468133323)*u**16+(4225.176250205621)*u**12+(-4135.631679358189)*u**15+(3150.629368575845)*u**33+(-2242.465868645306)*u**10+(1905.9703910228877)*u**9+(-1780.8574563843702)*u**34+(945.4954759182868)*u**35+(-625.1414269646766)*u**8+(-470.7762209120602)*u**36+(273.64582802888583)*u**6+(219.45734240375398)*u**37+(-187.6931562060072)*u**7+(-117.30607315976239)*u**5+(111.9036513814216)*u**11+(-95.60007919270967)*u**38+(62.120581108731955)*u**4+(-55.088366100784846)*u**3+(38.839824200251904)*u**39+(19.845409090037467)*u**2+(-14.68594151818552)*u**40+(5.156917050403481)*u**41+(-1.6779008979467922)*u**42+(1.3825087750779976)*u+(0.504677646451718)*u**43+(-0.13997762976368017)*u**44+(-0.03926806309183914)*1+(0.03570496074490221)*u**45+(-0.008350014963907736)*u**46+(0.001783695213018054)*u**47+(-0.00034639270635900265)*u**48+(6.077079706782282e-05)*u**49+(-9.549758692773345e-06)*u**50+(1.3286553330166033e-06)*u**51
    ...
    def a_real_1(self, u):
        a = (0.2179491366100223)*u+(-0.02578314605543316)*1
        return torch.abs(a).double()

    def a_real_2(self, u):
        a = (-0.15302356844668089)*u+(0.05183060167296635)*1
        return torch.abs(a).double()

    def a_real_3(self, u):
        a = (0.7307859944648574)*u+(-0.368821428971241)*1
        return torch.abs(a).double()

    def a_real_4(self, u):
        a = (-0.3634553050035578)*u+(0.05501603421144189)*1
        return torch.abs(a).double()
...
if __name__ == "__main__":
    experiment_name = 'beta_300_case_2_N_200_0.5_obs_2'
