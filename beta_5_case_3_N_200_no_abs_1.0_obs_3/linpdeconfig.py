...
def loss(model, stepnum, obs_data, layerweight=None):
    ...
    x0_file = 'data/beta_5_case_3_N_200_1.0_obs_3_x0.npy'
    ...
    a = 1.0 * model.a_predict(u_fixed_a)
    # amin
    amin = torch.min(a)
    # amax
    amax = torch.max(a)
    ...
    # 位置观测点
    obs_points = []
    for i in range(0, 199, 15):
        obs_points.append(i)

    amax_up_bound = 10.0
    fmax_up_bound = 25.0
    if 0 < max_f_prime < fmax_up_bound and 0 < amax < amax_up_bound and penalty == 0:
        print("mormal")
        loss = mse(obs_data_choose[:, :, :], pre_data_choose[:, :, :])
    elif 0 < max_f_prime < fmax_up_bound and (amax <= 0.0 or amax >= amax_up_bound) and penalty == 0:
        print("a#loss")
        loss = torch.abs(amax - amax_up_bound) + torch.abs(amax)
    elif (max_f_prime <= 0 or max_f_prime >= fmax_up_bound) and 0.0 < amax < amax_up_bound and penalty == 0:
        print("f#loss")
        loss = torch.abs(max_f_prime - fmax_up_bound) + torch.abs(max_f_prime)
    else:
        print("f#a#loss")
        loss = torch.abs(max_f_prime - fmax_up_bound) + torch.abs(max_f_prime) + torch.abs(amax - amax_up_bound) + torch.abs(amax) + penalty
    ...
    print("\033[33m loss0 %.6f, max_f_prime %.6f, amin %.6f, amax %.6f, penalty %.6f \033[0m" % (
            loss, max_f_prime, amin, amax, penalty))
