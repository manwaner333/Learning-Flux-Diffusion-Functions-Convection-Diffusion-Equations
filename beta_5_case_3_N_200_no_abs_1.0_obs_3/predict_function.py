...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def f_real(self, u):
        ...
        # a = (0.7111187800088717)*u**2+(0.7071747326731231)*u**3+(0.552516964257862)*u+(-0.46797455895681656)*1+(-0.3158258586129421)*u**4+(0.0013628331827317374)*u**5+(-1.7843912073613243e-06)*u**6
        # b = (4.052696598062573)*u**2+(-3.2300204991280186)*u+(0.9849135530164465)*u**3+(0.9679638648845688)*1+(-0.4428170059458697)*u**4+(0.0019108308302399925)*u**5+(-2.5018980866026857e-06)*u**6
        # a = (8.94967735009678)*u**6+(6.320658319460378)*u**5+(5.292084137960746)*u**7+(2.122328221686811)*u**4+(1.8373929791902128)*u**3+(1.4765712801372972)*u**2+(1.1339768865044928)*u**8+(-0.529989445465226)*1+(-0.3334131388278525)*u
        # b = (13.77910191079441)*u**6+(9.731411728067858)*u**5+(8.147798384784359)*u**7+(4.167543534244692)*u**3+(3.8412680650740905)*u**4+(-2.7925128250916873)*u+(1.745893452065262)*u**8+(1.4449123811456925)*1+(1.153944584290914)*u**2
        a = (8.94967735009678)*u**6+(6.320658319460378)*u**5+(5.292084137960746)*u**7+(2.122328221686811)*u**4+(1.8373929791902128)*u**3+(1.4765712801372972)*u**2+(1.1339768865044928)*u**8+(-0.529989445465226)*1+(-0.3334131388278525)*u
        b = (13.77910191079441)*u**6+(9.731411728067858)*u**5+(8.147798384784359)*u**7+(4.167543534244692)*u**3+(3.8412680650740905)*u**4+(-2.7925128250916873)*u+(1.745893452065262)*u**8+(1.4449123811456925)*1+(1.153944584290914)*u**2
        return a/b
    ...
    def a_real_1(self, u):
        a = (-0.4130972652563325)*u+(0.0253715893658325)*1
        return torch.abs(a).double()

    def a_real_2(self, u):
        a = (-0.011627725688457364)*u+(0.004564270437174817)*1
        return torch.abs(a).double()

    def a_real_3(self, u):
        a = (0.7337937808511976)*u+(-0.3773930973155576)*1
        return torch.abs(a).double()

    def a_real_4(self, u):
        a = (0.3651005745768202)*1+(-0.0627717761022562)*u
        return torch.abs(a).double()
