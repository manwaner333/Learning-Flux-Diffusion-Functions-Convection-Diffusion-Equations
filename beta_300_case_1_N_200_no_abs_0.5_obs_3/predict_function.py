...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def f_real(self, u):
        # f = (-659702.7185050546)*u**28+(654637.511112061)*u**27+(645160.6100591067)*u**29+(-644664.0946044867)*u**26+(638021.3622872028)*u**25+(-630235.0022807303)*u**24+(606055.5271849302)*u**23+(-599130.4245880753)*u**30+(-550300.0082787053)*u**22+(519629.97379603575)*u**31+(460308.5344064221)*u**21+(-416178.81165202707)*u**32+(-349300.5490508509)*u**20+(305602.4610643125)*u**33+(237764.74611341322)*u**19+(-204857.474120799)*u**34+(-141800.89811012056)*u**18+(125063.21676988965)*u**35+(-69454.07879467815)*u**36+(68612.0908114546)*u**17+(35075.06939710257)*u**37+(-19385.367543723445)*u**16+(-16108.806600486232)*u**38+(15464.080068979953)*u**14+(-11822.653104223973)*u**13+(-7339.727195522068)*u**15+(6729.955225360986)*u**39+(4387.870413804449)*u**12+(-2714.6843721120317)*u**10+(-2558.4933713726896)*u**40+(1955.8812262797421)*u**9+(1042.6721488499763)*u**11+(885.301056200994)*u**41+(-743.317518015473)*u**8+(-278.8574058205888)*u**42+(101.57007171434005)*u**6+(-86.00084126808794)*u**5+(79.95066342145586)*u**43+(78.79796590189974)*u**4+(-64.01641670839308)*u**3+(63.969335857201514)*u**7+(21.454521812458378)*u**2+(-20.857812933371026)*u**44+(4.948271536619144)*u**45+(1.2382616204413168)*u+(-1.0664805248606393)*u**46+(0.20851825159068402)*u**47+(-0.20751239606808272)*1+(-0.03690857441367107)*u**48+(0.005896510260742671)*u**49+(-0.0008464896118873939)*u**50+(0.00010847842616062873)*u**51+(-1.2288238413486834e-05)*u**52+(1.2124657361421457e-06)*u**53
        f = (66421.4118566256)*u**28+(65178.044212212575)*u**21+(-64928.00908003173)*u**20+(-64398.06105846598)*u**29+(-60602.4191500477)*u**27+(56760.75809958727)*u**30+(55671.44067967981)*u**19+(-54388.02726880888)*u**22+(-45980.79896159293)*u**31+(45704.91078913276)*u**26+(-40861.2229857805)*u**18+(34332.43573168292)*u**32+(33264.74442494944)*u**23+(24299.791318145457)*u**17+(-23606.170891302005)*u**33+(-22476.31580984383)*u**25+(14912.013379635293)*u**34+(-9573.570446382571)*u**16+(-8633.702834344189)*u**35+(-5694.260523594717)*u**24+(4923.548038841936)*u**14+(-4588.843188306411)*u**13+(4572.63086253846)*u**36+(-2212.330074348854)*u**37+(2032.5393450176482)*u**12+(-1221.404279605525)*u**10+(989.6215688975959)*u**9+(976.9412183532254)*u**38+(-540.4758581062612)*u**15+(-409.7767846914912)*u**8+(-393.54306194906854)*u**39+(292.1717899801514)*u**11+(144.57022126483807)*u**40+(55.859372817804164)*u**6+(52.99721245715598)*u**4+(-51.53008652487216)*u**3+(-48.419237067933814)*u**41+(-44.80869219164241)*u**5+(37.34986422496874)*u**7+(17.720010256037302)*u**2+(14.78045857072267)*u**42+(-4.110749352813037)*u**43+(1.6841825910039323)*u+(1.0410565592995358)*u**44+(-0.26396004342830265)*1+(-0.23988441457475992)*u**45+(0.050236807210104255)*u**46+(-0.009547001572679107)*u**47+(0.0016428727580458458)*u**48+(-0.0002552117791876214)*u**49+(3.5629812390841766e-05)*u**50+(-4.440846004454221e-06)*u**51
        return f
    ...
    def a_real_1(self, u):
        # a = (0.13620270413662972)*u+(0.0056104237057651215)*1
        a = (0.12392774057257737)*u+(-0.012357270382967828)*1
        return torch.abs(a).double()

    def a_real_2(self, u):
        # a = (-0.08002183771089351)*u+(0.027915798297774328)*1
        a = (-0.09383728698826124)*u+(0.005656356935054112)*1
        return torch.abs(a).double()

    def a_real_3(self, u):
        # a = (1.1557177410991395)*u+(0.3128067747728934)*1
        a = (1.1672482346820114)*u+(0.3317831322851484)*1
        return torch.abs(a).double()

    def a_real_4(self, u):
        # a = (-0.6765229581490037)*u+(-0.329392436516399)*1
        a = (-0.6147395143123183)*u+(-0.25997089236418663)*1
        return torch.abs(a).double()
...
if __name__ == "__main__":
    experiment_name = 'beta_300_case_1_N_200_0.5_obs_3'
