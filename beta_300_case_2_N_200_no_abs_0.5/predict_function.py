...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def f_real(self, u):
        f = (-8183.770932265343)*u**22+(7942.120365367404)*u**21+(7856.290650034703)*u**23+(-7190.667924074114)*u**20+(-7021.796678970682)*u**24+(6060.735107787768)*u**19+(5846.6826472881485)*u**25+(-4656.077683079492)*u**18+(-4541.766487285799)*u**26+(3297.373473350415)*u**27+(-3282.8577992794876)*u**13+(2976.3792557867396)*u**17+(2886.106663107083)*u**14+(-2241.3733900935263)*u**28+(2082.10586627609)*u**12+(1428.7155975694975)*u**29+(-1181.3408928800998)*u**15+(-1114.1353483555408)*u**10+(1109.977518301693)*u**9+(-976.9678397270492)*u**16+(-855.0706882883101)*u**30+(480.8984751366409)*u**31+(-404.7216934590031)*u**8+(-254.2692783422666)*u**32+(201.26843150818786)*u**6+(-151.07095047267032)*u**11+(-132.1637392565154)*u**7+(126.3991389053835)*u**33+(-80.8231048844813)*u**5+(-59.05561098453098)*u**34+(-44.62954041518101)*u**3+(41.5486949369707)*u**4+(25.915016624422133)*u**35+(17.059894052731654)*u**2+(-10.670487395752652)*u**36+(4.1172157316081375)*u**37+(1.6489773986754006)*u+(-1.4864114945537368)*u**38+(0.5012023132120342)*u**39+(-0.15752356564581835)*u**40+(0.0460418176153553)*u**41+(-0.037551582259574105)*1+(-0.012483445360370462)*u**42+(0.0031307340419885984)*u**43+(-0.0007238229773961218)*u**44+(0.00015364228415091848)*u**45+(-2.9783624725119437e-05)*u**46+(5.2349462016828515e-06)*u**47
        return f
    ...
    def a_real_1(self, u):
        a = (0.09175786292197967)*u+(-0.021807865265482652)*1
        return torch.abs(a).double()

    def a_real_2(self, u):
        a = (-0.10461771181855767)*u+(0.037519644709899325)*1
        return torch.abs(a).double()

    def a_real_3(self, u):
        a = (0.6963932156214532)*u+(-0.3468875313789374)*1
        return torch.abs(a).double()

    def a_real_4(self, u):
        a = (-0.38830850126116734)*u+(0.0342355687619051)*1
        return torch.abs(a).double()
...
if __name__ == "__main__":
    experiment_name = 'beta_300_case_2_N_200_0.5_obs_1'
